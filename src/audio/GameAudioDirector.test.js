...
describe('GameAudioDirector', () => {
  ...
  it('restores and persists independent clamped settings', () => {
    const saved = storage({ 'pokerStrike.audio.v1': JSON.stringify({ bgmVolume: 0.3, sfxVolume: 0.7, muted: true }) });
    ...
    expect(JSON.parse(saved.getItem('pokerStrike.audio.v1'))).toEqual({ bgmVolume: 1, sfxVolume: 0, muted: false });
  });
