...
- 강화 선택 제목은 단순 텍스트 대신 `battle-label-frame.png` 톤의 배너형 프레임, 영문 보조 라벨, 안내 문구로 구성한다.
- 강화 선택지 카드는 더 넓고 높게 조정해 긴 강화명과 적용 대상 문구가 카드 밖으로 삐져나가지 않게 한다.
- 강화명과 적용 대상 텍스트는 카드 내부 래핑 폭을 별도로 제한해 중앙 정렬을 유지한다.
