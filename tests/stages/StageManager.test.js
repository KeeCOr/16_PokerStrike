...
describe('StageManager', () => {
  it('makes the first wave easier than the stage baseline', () => {
    ...
    expect(BASE_ENEMY_COUNT_MULTIPLIER).toBe(6);
    expect(getWaveEnemyCountMultiplier(0, 0, STAGES[0].waves.length)).toBeCloseTo(3.3);
  ...
  it('scales enemy count exponentially by stage', () => {
    expect(getStageEnemyCountMultiplier(0)).toBe(6);
    expect(getStageEnemyCountMultiplier(4)).toBe(20);
    expect(getStageEnemyCountMultiplier(9)).toBe(89);
  });
  ...
  it('makes later waves harder than the stage baseline', () => {
    ...
    expect(getWaveEnemyCountMultiplier(stageIndex, waveIndex, STAGES[stageIndex].waves.length)).toBeCloseTo(9.6);
